...
/*************如果找到了设备则进行下载spiflash，如果没有则先下载SDK中的cyfxflashpro.img去RAM*************/
...
my_fx3_usbboot_download (
	...
	return fx3_usbboot_download(h,filename);
...
bool Updata_FX3(libusb_device *dev,int position)
{
	...
	string str = "CyBootProgrammer.img";
	char * cstr = new char[str.length()+1];
	strcpy(cstr, str.c_str());

	const char * filename = name_UpdateFile;

	switch (position) {
		case 1:				//RAM
			cout << "waiting download image: " << str <<endl;
			r = my_fx3_usbboot_download (handle, cstr);
			if(r == 0)
				cout << str << " download succeed"<<endl;
			break;
		case 2:				//I2CE2PROM
			r = fx3_i2cboot_download (handle, filename);
			break;
		case 3:          	//SPIFLASH
			r = fx3_spiboot_download (handle, filename);
			break;
		default:
			break;
	}
